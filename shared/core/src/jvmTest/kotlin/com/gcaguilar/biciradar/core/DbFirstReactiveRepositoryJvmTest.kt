...
class DbFirstReactiveRepositoryJvmTest {
  ...
  @Test
  fun `surface snapshot refresh prefers persisted station cache over lagging state flow`() =
    runTest {
      val database = createTestDatabase()
      StationCacheStore(database).save(
        cityId = City.ZARAGOZA.id,
        stations =
          listOf(
            Station(
              id = "station-db-cache",
              name = "Station DB Cache",
              address = "Centro",
              location = GeoPoint(41.65, -0.88),
              bikesAvailable = 7,
              slotsFree = 3,
      ...
      val repository =
        SurfaceSnapshotRepositoryImpl(
          ...
          favoritesRepository =
            object : FavoritesRepository {
              override val favoriteIds = MutableStateFlow(setOf("station-db-cache"))
              override val homeStationId = MutableStateFlow<String?>(null)
              override val workStationId = MutableStateFlow<String?>(null)
