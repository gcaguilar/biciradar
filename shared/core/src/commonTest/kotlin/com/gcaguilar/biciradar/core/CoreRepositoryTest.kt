...
class CoreRepositoryTest {
  ...
  @Test
  fun `favorites repository persists ids to disk`() =
    runTest {
  ...
  @Test
  fun `settings repository persists search radius`() =
    runTest {
  ...
  @Test
  fun `settings repository persists preferred map app`() =
    runTest {
  ...
  @Test
  fun `favorites repository merges local and wearable ids on bootstrap`() =
    runTest {
  ...
  @Test
  fun `favorites repository ignores corrupt local snapshot during bootstrap`() =
    runTest {
  ...
  @Test
  fun `favorites repository pushes merged ids back to wearable on bootstrap`() =
    runTest {
  ...
  @Test
  fun `favorites repository syncFromPeer pulls newer wearable ids`() =
    runTest {
      ...
      val watchBridge =
        object : WatchSyncBridge {
          ...
          var remoteSnapshot: FavoritesSyncSnapshot? =
            FavoritesSyncSnapshot(
              favoriteIds = setOf("station-watch"),
            )
  ...
  @Test
  fun `favorites repository pushes updated ids on toggle`() =
    runTest {
  ...
  @Test
  fun `favorites bootstrap does not block when wearable sync is slow`() =
    runTest {
  ...
  @Test
  fun `favorites repository persists home and work stations`() =
    runTest {
  ...
  @Test
  fun `favorites repository deduplicates home and work stations`() =
    runTest {
  ...
  @Test
  fun `favorites repository keeps custom categories assigned to a single station`() =
    runTest {
  ...
  @Test
  fun `favorites repository merges remote home and work aliases`() =
    runTest {
  ...
  @Test
  fun `stations repository refresh uses current user location when available`() =
    runTest {
  ...
  @Test
  fun `stations repository falls back to default location when current location times out`() =
    runTest {
      var requestedOrigin: GeoPoint? = null
    ...
    val graph =
      CoreGraph.Companion.create(
        object : PlatformBindings {
          ...
          override val httpClientFactory: BiziHttpClientFactory =
            object : BiziHttpClientFactory {
  ...
  @Test
  fun `assistant resolver highlights nearest station and favorite count`() =
    runTest {
      ...
      assertEquals(
        "Nearest station with bikes: Plaza Espana. 7 bikes available, 4 slots free",
        nearestWithBikes.spokenResponse,
      )
      assertEquals(
        "Nearest station with slots: Plaza Aragón. 0 bikes available, 9 slots free",
  ...
  @Test
  fun `assistant resolver returns missing status when station is unknown`() =
    runTest {
      val resolution =
        DefaultAssistantIntentResolver().resolve(
  ...
  @Test
  fun `assistant resolver returns bike and slot counts for known station`() =
    runTest {
      val station =
        Station(
          id = "station-42",
  ...
  @Test
  fun `assistant resolver falls back to nearest outside configured radius`() =
    runTest {
      val stations =
        listOf(
          Station(
      ...
      assertEquals(
        "No stations with bikes within 500m. Nearest is 720m away: Plaza Aragón. 3 bikes available, 5 slots free",
        resolution.spokenResponse,
      )
    }
