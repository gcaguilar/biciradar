...
For Android mobile, the workflow publishes `alpha` with `status: completed` and a configurable `update_criticality` value mapped to Play's `inAppUpdatePriority`.
